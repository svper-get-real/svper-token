...
const svpTokenArtifacts = require('../build/contracts/SVPToken.json');
const contract = require('truffle-contract');
let SvpToken = contract(svpTokenArtifacts);
...
SvpToken.setProvider(web3.currentProvider);

let svpTokenAddress = process.argv.slice(2)[0];
...
async function setAllocation () {
  ...
  let svpToken = await SvpToken.at(svpTokenAddress);
  ...
  for (var i = 0; i < distribAddressData.length; i++) {
    try {
      ...
      let r = await svpToken.batch(distribAddressData[i], distribAmountData[i], { from: walletProvider.getAddress(), gas: 4500000, gasPrice: 2000000000 });
      console.log('---------- ---------- ---------- ----------');
...
if (svpTokenAddress) {
  console.log('Processing SVPToken allocation. Batch size is', BATCH_SIZE, 'accounts per transaction');
  readFile();
} else {
  console.log('Please run the script by providing the address of the SVPToken contract');
}